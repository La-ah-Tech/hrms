...
def get_custom_fields():
    """HR specific custom fields that need to be added to the masters in LaahTech-EM"""
    return {
        "Company": [
            {
...
def setup_notifications():
    ...
    response = frappe.read_file(
        os.path.join(
            base_path, "leave_application/leave_application_email_template.html"
        )
    )
    records = [
        {
    ...
    response = frappe.read_file(
        os.path.join(
            base_path, "interview/interview_reminder_notification_template.html"
        )
    )
    records += [
        {
    ...
    response = frappe.read_file(
        os.path.join(
            base_path, "exit_interview/exit_questionnaire_notification_template.html"
        )
    )
    records += [
        {
...
def update_hr_defaults():
    ...
    hr_settings.exit_questionnaire_notification_template = _(
